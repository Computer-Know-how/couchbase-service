...
class CouchbaseService {
	...
	getCallback(key, callback) { // callback: (error, value)
		return this.bucketCall('get', [key], (error, result) => callback(error, result.value));
	}
	...
	getMultiPromise(keys) {
		return new Promise((resolve, reject) => {
			this.getMultiCallback(keys, (error, result) => error ? reject(error) : resolve(result));
		});
	}
	...
	prepareViewQuery(ddoc, name, options) {
		...
		// The following options are valid view query options
		const validOptions = [
			'custom',
			'group',
			'group_level',
		...
		for (const [option, opt] of Object.entries(options)) {
			switch (option) {
				...
				default:
					// Use a try/catch to handle invalid option errors
					// This is preferred to checking the validOptions array every time since
					//   invalid options errors will only occur during development
					try {
						query[option](opt);
					} catch (e) {
						// If the error was because of an invalid option,
		...
		for(let opt in query.options) {
			if(query.options.hasOwnProperty(opt) && query.options[opt] === undefined) {
				delete query.options[opt];
			}
		}
