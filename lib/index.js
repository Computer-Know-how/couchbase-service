...
class CouchbaseService {
	...
	*/
	viewQueryCallback(ddoc, name, options, callback) {
		...
		if(options.order) {
			options.order = (options.order === 'descending') ? 'true' : 'false';
		}
	...
	/**
		...
		* @param {Function} callback: (error, result, meta)
	*/
	n1qlQueryCallback(qry, options={}, callback) {
		this.bucketCall(
			'query',
			[qry, options],
			callback
	...
	*/
	upsertDesignDocumentCallback(name, views, development=false, callback) {
		try {
			const manager = this.bucket.viewIndexes();

			manager.upsertDesignDocument(
				{
					name: development ? `dev_${name}` : name,
					views
				},
				{},
				callback
	...
	*/
	viewQueryPromise(ddoc, name, options) {
		...
		return new Promise(async(resolve, reject) => {
			try {
				const viewResult = await this.bucket.viewQuery(ddoc, name, options);
	...
	*/
	...
	/**
		* @function dropDesignDocumentPromise - drops an existing design document (view query)
		* @param {String} name - design document name
		* @param {Boolean} development - if true, install design document in dev documents
		* @returns {Promise}
	*/
	dropDesignDocumentPromise(name, development=true) {
		return new Promise((resolve, reject) => {
			const manager = this.bucket.viewIndexes();

			manager.dropDesignDocument(
...
function mapErrorCodes(error) {
	if(error && error.constructor) {
		if(error.constructor.name === 'DocumentNotFoundError') {
			error.code = 13;
		} else if(error.constructor.name === 'DocumentExistsError') {
			error.code = 12;
		} else if(['TemporaryFailureError', 'DocumentLockedError'].includes(error.constructor.name)) {
			error.code = 11;
