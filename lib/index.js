...
class CouchbaseService {
	constructor (bucketName, configOptions) {
		...
		this.cluster = new Cluster(configOptions.cluster, {username:configOptions.auth.username, password: configOptions.auth.password});
		// Open the Couchbase bucket
		this.bucket = this.cluster.bucket(bucketName, error => {
	 ...
	 */
	async n1qlQueryCallback(qry, callback) { // callback: (error, result, meta)
		console.log(qry);
	...
	mapErrorCodes(error) {
		if(error instanceof couchbase.DocumentNotFoundError) {
			...
			error.code = 12;
		} else if(error instanceof couchbase.TemporaryFailureError){
			error.code = 11;
			...
			op.attempt(() => {
				...
				if(!['counter', 'query'].includes(method)) {
					this.collection[method](...args, (error, result, meta) => {
						...
						if (method === 'upsert' || method === 'insert' || method === 'replace') {
					...
					console.log(Object.values(this.cluster));
					return this.bucket.viewQuery()(...args , (error, result, meta) => {
						console.log('this.bucket.viewQuery',error);
						console.log('this.bucket.viewQuery',result);
						const delay = error === 11 ? 500 : 0;
						setTimeout(() => {
							if (error && [11, 16, 23].includes(error.code)) {
								if (typeof this.onReconnectCallback === 'function') {
