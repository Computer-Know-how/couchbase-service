		...
		try {
			...
			couchbase.connect(configOptions.cluster, configOptions.auth, (error ,cluster) => {
				...
				if (this.cluster) {
					...
					return configOptions.onConnectCallback(new Error('No bucket found, could not connect to Couchbase'));
				}
					...
					try {
						if (x++ >= 16) {
							...
							if (this.cluster._conns[bucketName]._connected === false) {
								return configOptions.onConnectCallback(new Error('could not connect to Couchbase'));
							} else {
